...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  box/.style={draw, rounded corners=2pt, minimum width=2.45cm, minimum height=1.0cm,
              align=center, font=\zihao{-5}},
  arr/.style={-{Stealth[length=2.5mm]}, thick},
  every node/.style={font=\zihao{-5}}
]
  \node[box] (d1) {原始数据集\\3713 张图像};
  \node[box, right=0.6cm of d1] (d2) {分层抽样划分\\训练 2806 + 验证 494};
  \node[box, right=0.6cm of d2] (d3) {负样本构造\\600 张无破损图像};
  \node[box, right=0.6cm of d3] (d4) {数据增强\\HSV/Mosaic/Copy-Paste};
  \node[box, right=0.6cm of d4] (m1) {YOLOv8s\\检测模型};

  \node[box, below=1.0cm of m1] (t1) {测试集 413 张};
  \node[box, below=1.0cm of d1] (r1) {test\_result.csv\\定位与类别};
  \node[box, below=1.0cm of r1] (r2) {最大置信度 $s_i$};
  \node[box, below=1.0cm of r2] (r3) {Weibull 拟合};
  \node[box, below=1.0cm of r3] (r4) {缺陷概率 $P_{\mathrm{defect}}$};
  \node[box, below=1.0cm of r4] (r5) {二分类判别结果};

  \node[box, below=1.0cm of d4] (e1) {多维度评估};

...
YOLOv8 是 Ultralytics 提出的单阶段目标检测框架\cite{jocher2023}，采用 anchor-free 解耦检测头设计，整体由 Backbone、Neck 与 Head 三部分组成，其结构示意如图 \ref{fig:yolo} 所示。Backbone 以 C2f 模块堆叠提取多尺度特征，并经 SPPF 池化增强感受野；Neck 采用 PAN-FPN\cite{liu2018panet} 结构实现自顶向下与自底向上的双向多尺度特征融合，输出 P3、P4、P5 三组特征图；Head 对每个位置直接预测类别与包围框（anchor-free），并通过解耦分支分别进行回归与分类，最后经非极大值抑制（NMS）输出检测结果。
...
\begin{equation}
  ...
  \label{eq:total_loss}
\end{equation}
其中定位损失采用 CIoU 损失。对预测框 $B$ 与真实框 $B^{\mathrm{gt}}$，记两者交集与并集的面积分别为 $|B\cap B^{\mathrm{gt}}|$ 与 $|B\cup B^{\mathrm{gt}}|$，则交并比（IoU）\cite{lin2014coco}定义为
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:ciou}
\end{equation}
其中 $\rho(\cdot)$ 为两框中心点的欧氏距离，$c$ 为最小外接矩形对角线长度，
$v=\frac{4}{\pi^{2}}\bigl(\arctan\frac{w^{\mathrm{gt}}}{h^{\mathrm{gt}}}-\arctan\frac{w}{h}\bigr)^{2}$
度量预测框与真实框的长宽比一致性，$\alpha=\frac{v}{(1-\mathrm{IoU})+v}$ 为平衡系数\cite{zheng2020diou}。分类损失采用二元交叉熵：
\begin{equation}
...
三组实验在验证集上的最优指标如表 \ref{tab:det_compare} 所示。改进模型相比基线，mAP@0.5 由 0.393 提升至 0.413，mAP@0.5:0.95 由 0.190 提升至 0.212，提升幅度约 11.5\%。为厘清增益来源，本文补充了两组对照：在同配置下将 Copy-Paste 概率分别设为 0 与 0.5 训练，两条 150 轮曲线逐轮完全一致（含训练损失），说明 Copy-Paste 在本数据集（仅目标框标注、无分割掩膜）上未产生可观测增益，上述提升主要来源于骨干网络容量升级（YOLOv8n$\to$YOLOv8s）。最终模型在纯正样本验证集上 mAP 略低于改进模型（mAP@0.5 为 0.403、mAP@0.5:0.95 为 0.205），这是因为负样本训练使模型对低置信度背景响应更加保守，但显著增强了背景抑制能力与问题一判别效果，属于以微小精度代价换取开放集鲁棒性的合理权衡。
...
\begin{tabular}{lcccc}
  ...
  \midrule
  基线（YOLOv8n） & 0.393 & 0.190 & 0.546 & 0.402 \\
  改进（YOLOv8s+Copy-Paste） & 0.413 & 0.212 & 0.601 & 0.414 \\
  最终模型（+负样本） & 0.403 & 0.205 & 0.564 & 0.407 \\
  \bottomrule
\end{tabular}
  \vspace{2pt}
  {\footnotesize 注：表中指标为按验证集 mAP@0.5:0.95 最优轮次选取的同一轮结果（基线 ep99、改进 ep139、最终模型 ep130）；最终模型另有独立复测 mAP@0.5=0.403、mAP@0.5:0.95=0.204（表 \ref{tab:per_class}）。}
\end{table}
...
\begin{enumerate}[leftmargin=2.2em,itemsep=2pt]
  ...
  \item {\heiti P2 高分辨率检测头：}采用 \texttt{yolov8s-p2}（新增 $160\times160$ 特征图检测头）训练，验证集 mAP@0.5:0.95 为 0.209；Hole 类 mAP@0.5:0.95 由 0.190 升至 0.210（+0.021）、召回率由 0.407 升至 0.453，小目标收益显著，但整体收益有限（+0.005）；
  \item {\heiti 测试时增强（TTA）：}对最终模型启用多尺度测试增强，mAP@0.5:0.95 由 0.204 升至 0.215（+0.011），三类别均提升，且无需重新训练，是性价比最高的改进方向。
\end{enumerate}

\subsubsection{检测结果可视化}

最终模型在验证集上的 PR 曲线与归一化混淆矩阵分别如图 \ref{fig:pr} 与图 \ref{fig:cm} 所示。PR 曲线显示 Dent 类精度-召回权衡最优，Hole 类次之，Rusty 类相对偏弱；混淆矩阵表明主要误检发生在 Rusty 与背景之间。图 \ref{fig:val_labels} 与图 \ref{fig:val_pred} 给出同一批验证图像的标注与预测对比，图 \ref{fig:detect_samples} 给出测试集检测结果示例，模型能够准确定位多类缺陷并输出类别与置信度。

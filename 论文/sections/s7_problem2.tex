...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  block/.style={draw, rounded corners=2pt, minimum height=0.9cm, align=center, font=\zihao{-5}},
  arr/.style={-{Stealth[length=2.5mm]}, thick},
]
  \node[block, minimum width=2.2cm] (in) {输入图像\\$640\times640\times3$};
  \node[block, right=0.6cm of in, minimum width=2.9cm] (bb) {Backbone\\Conv + C2f + SPPF};
  \node[block, right=0.6cm of bb, minimum width=2.9cm] (nk) {Neck\\PAN-FPN 双向融合};
  \node[block, right=0.6cm of nk, minimum width=2.6cm] (hd) {检测头\\P3/P4/P5 解耦头};
  \node[block, right=0.6cm of hd, minimum width=2.0cm] (nm) {NMS\\后处理};
  \node[block, right=0.6cm of nm, minimum width=2.6cm] (out) {输出\\位置+类别+置信度};

  \node[block, below=0.55cm of hd, minimum width=4.6cm] (cls) {分类分支：$C$ 类置信度};
  \node[block, below=0.55cm of cls, minimum width=4.6cm] (reg) {回归分支：$4$ 维框坐标};
...
\begin{equation}
  ...
  \label{eq:wd_focal}
\end{equation}
其中 $\sigma$ 为 sigmoid 函数，$\tau$ 为温度参数。该损失以特征空间的几何可分性替代样本频率的逆比例加权，理论上对长尾分布更鲁棒。受竞赛时间限制，本文三组对照实验采用 Ultralytics 默认损失实现，WD-Focal Loss 的完整推导、实现与接口见附录 C，作为后续优化的候选方案。
